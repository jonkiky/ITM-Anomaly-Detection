...
<div id="googledoc" class="modal">
	...
	<div id="draggableJof" class="ui-widget-content"
		style="display: none; width: 50%; top: 95px; right: 0px; position: absolute">
		<div class="row" style="margin-bottom:0px;">
			<div class="row"  style="margin-bottom:0px;">
				...
				<li>
					...
					<div class="collapsible-body">
						...
						<div>
								...
								<textarea row="3"
								type="textarea" id="problem_text_area_input" 
								style=" width: 100%; margin: 0px; padding-left: 5px; z-index: auto; position: relative; height:60px;
				...
				<li>
					...
					<div class="collapsible-body">
						...
						<div>
								<span
									style="display: block; overflow: hidden;"> 
									<textarea row="3" id="understand_text_area_input" 
									style=" width: 100%; margin: 0px; padding-left: 5px; z-index: auto; position: relative; height: 60px;
   									font-size: 12.5px;  transition: none; background: transparent !important;"> </textarea></span>
						</div>
	...
	<script>
	    // global variables 
		var isSupertalk=true                                                   // check this is a super_talk
		var localdb =getUrlParameter('localdb');                               // localdb is super talk
		var community =getUrlParameter('community');                           // community is null, The URL does not contains the community information, 
		var community_id =getUrlParameter('community_id');                     // community id  is  super talk id   
		var type =0;                                                           // type works for permission control 
		var urname ='<%=session.getAttribute("username")%>';
		var id ='<%=session.getAttribute("id")%>';                              // user str_id, which is used in KF6
		var itmdb ='<%=session.getAttribute("ITMDatabase")%>';                  
		var domain =getUrlParameter('domain');
		var school =getUrlParameter('school');
		var country =getUrlParameter('country');
		var city =getUrlParameter('city');
		var project_id =getUrlParameter('project_id');                        // for super talk is 1
		var project_name ="";                    							 
		var thread_id =getUrlParameter('thread_id');
		var threadname ="";
		var isCrossProject = false;
		var super_token ='<%=session.getAttribute("super_token")%>'          // permission control only allow the user in this community 
